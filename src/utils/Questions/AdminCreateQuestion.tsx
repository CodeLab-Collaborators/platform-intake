...
const AdminCreateQuestion = () => {
	...
	const setUpTestEnVi = async () => {
		...
		await axios
			...
			.then((res: any) => {
				setShow(true);
				window.localStorage.setItem("showTest", "")
				console.log(res);
			});
	};
	...
	const fetchData = async () => {
		...
		await axios.get(newURL).then((res: any) => {
					...
					<>
						...
						<TestBox>
							...
							<TypedQuestion>
								<TypedQuestionHold>
									<MainQuestions>
										{dataOption?.map((props: any, i: number) => (
...
const Button = styled.button<{ col: string, bd: string }>`
	height: 40px;
	width: 100%;
	font-family: poppins;
	background-color: transparent;
